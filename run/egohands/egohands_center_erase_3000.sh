...
do
    ...
    -od=${output_dir} \
    --dataset=${DATASET} \
    --threshold=0.3 > ${log_dir}/${MutateName}_3000.log

    base_dir=./${data_dir}/working_dir/${MutateType}/${MutateName}_th03_3000
    mkdir -p $base_dir
    ...
    --label_dir=${LABELDIR} \
    --target_dir=${WORKDIR} \
    --dataset=${DATASET} >> ${log_dir}/${MutateName}_3000.log
...
do
    ...
    base_dir=./${data_dir}/working_dir/${MutateType}/${MutateName}_th03_3000
    python -u -m scripts.train.train --obj_path=${OBJPATH} --cfg_path=$CFGPATH --retrain=1 --gpu=$gpu_id >> ${log_dir}/${MutateName}_3000.log
done
